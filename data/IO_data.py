...
class IO_data(Dataset):
    ...
    def __init__(self, inputs, outputs, seq_len=None):
        ...
        self.u = inputs[0:self.nBatches * self.batch_len].reshape((self.batch_len, self.nBatches, -1), order='F').astype(np.float64)
        self.y = outputs[0:self.nBatches * self.batch_len].reshape((self.batch_len, self.nBatches, -1), order='F').astype(np.float64)
